...
import { MapPin, Clock, MonitorPlay, CalendarDays } from "lucide-react";
...
const TheatreShowtimes = ({ movie, onSelectShow }) => {
  ...
  useEffect(() => {
    const fetchSchedulingData = async () => {
      setLoading(true);
      try {
        // Fetch all necessary data to map the relationships
        ...
        // Filter shows for ONLY the selected movie
        const movieShows = showsRes.data.filter(
        ...
        // Auto-select the first available date if shows exist
        if (movieShows.length > 0) {
          const dates = [
            ...new Set(movieShows.map((s) => s.show_time.split("T")[0])),
          ].sort();
  ...
  if (loading)
    return (
      <p style={{ color: "#888", padding: "20px" }}>Loading showtimes...</p>
    );
  if (shows.length === 0)
    return (
  ...
  // Get unique dates for the date picker tabs
  const availableDates = [
    ...new Set(shows.map((s) => s.show_time.split("T")[0])),
  ].sort();

  // Filter shows by the selected date
  const showsOnDate = shows.filter((s) => s.show_time.startsWith(selectedDate));

  // Group the filtered shows by Theatre -> Screen
  const groupedData = {};
  ...
  showsOnDate.forEach((show) => {
    ...
    if (!groupedData[theatre.theatre_id].screens[screen.screen_id]) {
      groupedData[theatre.theatre_id].screens[screen.screen_id] = {
        screen,
        shows: [],
      };
    }
        ...
        {availableDates.map((date) => {
          ...
          return (
            <button
              ...
              style={isSelected ? activeDateTab : dateTab}
            >
          ...
          Object.values(groupedData).map(({ theatre, screens }) => (
            <div key={theatre.theatre_id} style={theatreCard}>
              <div style={theatreHeader}>
                ...
                <p style={theatreLocation}>
                  <MapPin size={14} /> {theatre.location}, {theatre.city}
                </p>
              </div>
              ...
              {Object.values(screens).map(({ screen, shows }) => (
                <div key={screen.screen_id} style={screenSection}>
                  <p style={screenName}>
                    <MonitorPlay size={14} /> {screen.screen_name}
                  </p>
                  <div style={timeGrid}>
                    {shows.map((show) => {
                      const time = new Date(show.show_time).toLocaleTimeString(
                        [],
                      ...
                      return (
                        <button
                          key={show.show_id}
                          onClick={() => onSelectShow(show, theatre, screen)}
                          style={timeBtn}
                        >
...
const priceText = { color: "#888", fontSize: "0.7rem", marginTop: "4px" };
